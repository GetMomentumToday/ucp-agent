...
function Stars({ rating }: { readonly rating: number }) {
  ...
  return (
    <span className={s.stars}>
      {Array.from({ length: full }, (_, i) => <span key={`f${i}`} className={s.starFull}>{'\u2605'}</span>)}
    ...
    <div className={s.checkoutCard}>
      {item && (
        <div className={s.checkoutItem}>
          ...
          {subtotal && <div className={s.checkoutItemPrice}>{formatCents(subtotal.amount, currency)}</div>}
        </div>
      )}
  ...
  return (
    <div className={s.orderCard}>
      ...
      <div className={s.orderTimeline}>
        <div className={`${s.timelineStep} ${s.timelineActive}`}><span className={s.timelineDot} /><span>Ordered</span></div>
        <div className={s.timelineLine} />
        <div className={s.timelineStep}><span className={s.timelineDot} /><span>Processing</span></div>
        <div className={s.timelineLine} />
        <div className={s.timelineStep}><span className={s.timelineDot} /><span>Shipped</span></div>
        <div className={s.timelineLine} />
        <div className={s.timelineStep}><span className={s.timelineDot} /><span>Delivered</span></div>
      </div>
      <div className={s.orderFooter}>
        <div className={s.orderTotal}>Total: <strong>{formatCents(totalEntry?.amount ?? 0, session.currency)}</strong></div>
...
function renderToolResult(toolName: string, result: unknown) {
  ...
  if (toolName === 'ucp_search_products' && isProductArray(result)) return <ProductCardsDemo products={result} />;
  if (toolName === 'ucp_complete_checkout' && isCheckoutSession(result) && result.order_id) return <OrderCardDemo session={result} />;
  if ((toolName === 'ucp_create_checkout' || toolName === 'ucp_update_checkout') && isCheckoutSession(result) && result.totals && result.totals.length > 0) return <CheckoutCard session={result} />;
  return null;
}
  ...
  return (
    <div className={s.onboarding}>
      <div className={s.onboardingCard}>
        ...
        <div className={s.onboardingStep}>{step + 1} / {ONBOARDING_STEPS.length}</div>
        ...
        <div className={s.onboardingDots}>
          {ONBOARDING_STEPS.map((_, i) => (
            <button key={i} className={`${s.onboardingDot} ${i === step ? s.onboardingDotActive : ''}`} onClick={() => setStep(i)} type="button" aria-label={`Step ${i + 1}`} />
          ))}
        </div>
        <div className={s.onboardingActions}>
          {step > 0 && <button onClick={() => setStep(step - 1)} className={`${s.sidebarBtn} ${s.btnSecondary}`} type="button">Back</button>}
          <button onClick={isLast ? onStart : () => setStep(step + 1)} className={`${s.sidebarBtn} ${s.btnPrimary}`} type="button">
...
function buildMessagesUpTo(stepIndex: number): RenderedMessage[] {
  ...
  for (let i = 0; i <= stepIndex && i < DEMO_SCRIPT.length; i++) {
    ...
    if (step.toolName) {
      msgs.push({ id: `msg-${i}-tool`, role: 'agent', toolName: step.toolName, toolState: 'done', toolResult: step.toolResult });
    } else {
      msgs.push({ id: `msg-${i}`, role: step.type === 'user' ? 'user' : 'agent', text: step.text, toolState: 'done' });
    }
  }
  return msgs;
}
...
export function DemoChat() {
  ...
  const advanceToStep = useCallback((targetStep: number) => {
    cleanup();
    ...
    if (step.toolName) {
      setMessages([...prevMessages, { id: `msg-${targetStep}-tool`, role: 'agent', toolName: step.toolName, toolState: 'running' }]);
      setTimeout(() => {
        setMessages([...prevMessages, { id: `msg-${targetStep}-tool`, role: 'agent', toolName: step.toolName!, toolState: 'done', toolResult: step.toolResult }]);
      }, 800);
    } else if (step.type === 'user') {
      setMessages([...prevMessages, { id: `msg-${targetStep}`, role: 'user', text: step.text, toolState: 'done' }]);
    } else {
      const fullText = step.text ?? '';
      setMessages([...prevMessages, { id: `msg-${targetStep}`, role: 'agent', text: fullText, toolState: 'done' }]);
      setAnimatingText('');
  ...
  const handleNext = useCallback(() => {
    ...
    if (next >= DEMO_SCRIPT.length) { setMessages(buildMessagesUpTo(DEMO_SCRIPT.length - 1)); setPhase('finished'); }
    else advanceToStep(next);
  }, [currentStep, advanceToStep, cleanup]);

  const handleBack = useCallback(() => {
    cleanup(); setAnimatingText(null);
    advanceToStep(Math.max(0, currentStep - 1));
  }, [currentStep, advanceToStep, cleanup]);

  const startGuided = useCallback(() => { setPhase('guided'); advanceToStep(0); }, [advanceToStep]);
  const resetDemo = useCallback(() => { cleanup(); setPhase('guided'); advanceToStep(0); }, [cleanup, advanceToStep]);

  const tooltip = currentStep >= 0 && currentStep < DEMO_SCRIPT.length ? DEMO_SCRIPT[currentStep]!.tooltip : null;
  ...
  return (
    <div className={s.page}>
      <div className={s.header}>
        <div className={s.logo}><span className={s.logoAccent}>Scout</span> by Momentum</div>
      </div>

      <div className={s.container}>
        <aside className={s.sidebar}>
          <div className={s.sidebarTitle}><span className={s.sidebarTitleDot} />Scout</div>
          ...
          <div className={s.convItem + ' ' + s.convActive}>
            ...
            <div><div className={s.convName}>Hiking jacket</div><div className={s.convPreview}>Sarah &middot; Under $100</div></div>
          </div>
          <div className={s.convItem}>
            <span className={s.convIcon}>{'\u{1F45F}'}</span>
            <div><div className={s.convName}>Trail shoes</div><div className={s.convPreview}>Mike &middot; Technical</div></div>
          </div>
          <div className={s.convItem}>
            <span className={s.convIcon}>{'\u{1F392}'}</span>
            <div><div className={s.convName}>Backpack 40L</div><div className={s.convPreview}>Anna &middot; Multi-day</div></div>
          </div>
          <div className={s.convNew}>+ New conversation</div>
          <div className={s.spacer} />
          <div className={s.debugItem}><span className={s.protocolBadge} style={{ fontSize: 9 }}>UCP v1.0</span></div>
        </aside>

        <div className={s.chatArea}>
          <div className={s.chatHeader}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span className={`${s.avatar} ${s.agentAvatar}`} style={{ margin: 0 }}>S</span>
              <div>
          ...
          <div className={s.messages}>
              ...
              if (msg.role === 'user') {
                return (
                  <div key={msg.id} className={`${s.msg} ${s.userMsg} ${dimClass}`}>
                    ...
                    <div className={`${s.bubble} ${s.userBubble} ${isHighlighted ? s.bubbleHighlight : ''}`}>
                      <div className={s.bubbleText}>{msg.text}</div>
                    </div>
                    {isHighlighted && tooltip && <Tooltip tooltip={tooltip} step={currentStep} total={DEMO_SCRIPT.length} isLast={isLastStep} isAnimating={animatingText !== null} onNext={handleNext} onBack={handleBack} onClose={resetDemo} />}
                  </div>
                );
              }
                ...
                <div key={msg.id} className={`${s.msg} ${s.agentMsg} ${dimClass}`}>
                  ...
                  {msg.toolName && (
                    <div className={s.toolIndicator}>
                      <span className={`${s.toolDot} ${msg.toolState === 'done' ? s.toolDotDone : s.toolDotRunning}`} />
                      {msg.toolState === 'done' ? 'called' : 'calling'} {msg.toolName}
                    </div>
                  )}
                  {msg.toolResult !== undefined && msg.toolResult !== null && msg.toolName !== undefined && (
                    <div className={isHighlighted ? s.resultHighlight : ''}>{renderToolResult(msg.toolName, msg.toolResult)}</div>
                  )}
                  {displayText !== undefined && (
                    <div className={`${s.bubble} ${s.agentBubble} ${isHighlighted ? s.bubbleHighlight : ''}`}>
            ...
            {phase === 'finished' && (
              <div className={s.tourInline}>
                ...
                <div className={s.tourDesc}>Scout completed a full purchase through UCP. This flow works with any connected store.</div>
                <div className={s.tourActions}>
                  <button onClick={resetDemo} className={`${s.tourBtn} ${s.tourBtnSecondary}`} type="button">Restart</button>
                  <button onClick={startGuided} className={`${s.tourBtn} ${s.tourBtnPrimary}`} type="button">{'\u21BB'} Replay</button>
                </div>
              </div>
            )}
      ...
      <div className={s.footer}>
        <span>Powered by</span><strong style={{ color: '#64748b' }}>Momentum</strong>
        <div className={s.footerDivider} /><span className={s.protocolBadge}>UCP</span>
        <div className={s.footerDivider} /><span>Magento &middot; Shopware &middot; Shopify</span>
      </div>
    </div>
  );
}
...
function Tooltip({ tooltip, step, total, isLast, isAnimating, onNext, onBack, onClose }: TooltipProps) {
  return (
    <div className={s.tourInline}>
      <div className={s.tourHeader}>
        <span className={s.tourStep}>{step + 1} of {total}</span>
        <button onClick={onClose} className={s.tourClose} type="button">{'\u2715'}</button>
      </div>
      <div className={s.tourTitle}>{tooltip.title}</div>
      <div className={s.tourDesc}>{tooltip.description}</div>
      <div className={s.tourActions}>
        <button onClick={onBack} disabled={step <= 0} className={`${s.tourBtn} ${s.tourBtnSecondary}`} type="button">Back</button>
